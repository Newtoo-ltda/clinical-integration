...
async function getAdmins() {
    try {
        ...
        console.error('Erro ao buscar admins:', error.response?.data || error.message);
        return [];
    }
}
...
async function fetchAppointments(adminId) {
    ...
    try {
        const url = `http://localhost:3000/schedules/${adminId}?date=${formattedDate}`;
        console.log(url);
...
const updateSchedule = async (id, updateScheduleDto) => {
    try {
        const response = await axios.patch(`http://localhost:3000/schedules/${id}`, updateScheduleDto);
...
async function reminderLoop(client, adminId) {
    ...
    try {
        ...
        const response = await axios.get(`http://localhost:3000/schedules/${adminId}?startDate=${today}&endDate=${tomorrow}`);
...
async function verificarEtiquetasUsuario(adminId, userSession, from, labels) {
    ...
    if (!client) {
        console.warn(`❌ Client não encontrado para admin ${adminId}`);
        return;
    }
